...
// Exactly one surface is active; every mode keeps its own layout, so
// switching back restores the widgets or pills that were set up before.
SettingsSection {
    id: section
    title: "Desktop mode"
    description: "One at a time: widgets on the wallpaper, a bar along one edge, or a notch at the top center. Applies to the active profile."
    ...
    SegmentedControl {
        ...
        onSelected: value => LayoutService.setMode(value)
    }
    RowLayout {
        Layout.fillWidth: true
