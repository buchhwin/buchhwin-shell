...
ShellRoot {
    ...
    FileView { id: future; path: Qt.resolvedUrl("../fixtures/layout-v3-future.json").toString().replace("file://", ""); blockLoading: true }
    ...
    Component.onCompleted: {
        ...
        const profile = result.config.profiles.minimal
        ...
        T.eq(plain.config.profiles.minimal.widgets.length, 1, "v1 screen without entries keeps default clock")
        ...
        const sane = Logic.sanitize({ activeProfile: "work", profiles: { work: {
        ...
        const saneWidget = sane.profiles.work.widgets[0]
        ...
        T.eq(sane.profiles.work.groups.map(group => group.id), ["g"], "empty groups removed")
        T.eq(sane.profiles.work.groups[0].members, ["a"], "unknown members removed")
        T.eq(sane.profiles.work.widgets[1].group, "", "dangling group reference cleared")
